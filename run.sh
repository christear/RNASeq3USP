...
echo overlapping introns with annotation ...
...
echo counting junctions spanning exon-intron bundary ...
...
if [ ! -d $out_dir/count ]; then
	echo creating directory $out_dir/count ... 
	mkdir $out_dir/count
else
	echo $out_dir/count exists 
	rm -f $out_dir/count/*.* 
fi

mv $out_dir/*.intron.left $out_dir/count/
mv $out_dir/*.intron.right $out_dir/count/
mv $out_dir/*.intron.left.summary $out_dir/count/
mv $out_dir/*.intron.right.summary $out_dir/count/

paste $out_dir/count/*.intron.left |awk '$1 ~ ":"' | awk '{for(i=7;i<=NF;i+=7) {printf("%s\t",$i)} printf("\n")}' > $out_dir/combined.intron.left.count
...
echo filtering splicing events within 3UTR and get final output ...
if [ ! -d $out_dir/out ]; then
	echo creating $out_dir/out ...
	mkdir $out_dir/out
else
